...
## Positioning the COLOSSUS v3 Serving Engine

---

## 1. Survey Scope & Methodology

This survey covers **42 papers** (2020–2026) organized into **7 categories** that collectively define the design space in which COLOSSUS v3 operates. For each paper we extract: core idea, method, evaluation setup, key results, weaknesses, and stated future work. We then perform cross-paper gap analysis to identify where COLOSSUS v3's contributions are truly novel.
...
These papers define the MoE model designs that COLOSSUS v3 serves. They are not competing systems — they are the *workloads*.
...
> [!WARNING]
> **Lossy vs. Lossless:** Papers #12, 27, 28, 29 all achieve lower latency by **degrading model quality** (dropping experts, reducing precision, or changing routing). COLOSSUS v3 is designed as **strictly lossless** — every column that the true router selects is fetched and computed, either via the fast path (prefetched) or slow path (demand-loaded).

---

## 6. Category C4: Neuron-Level & Activation Sparsity

These papers establish the **empirical foundation** for COLOSSUS v3's column-level decomposition.
...
> [!NOTE]
> **Key observation:** PowerInfer (paper #31) is the closest system to COLOSSUS v3's neuron-level philosophy, but it targets **dense models only**. No existing work combines neuron/column-level granularity with MoE expert offloading. This is COLOSSUS v3's core novelty.
...
| 44 | **AWQ** (Lin et al.) | 2024 | Activation-aware quantization | Protect salient weights based on activation | Complementary to COLOSSUS v3, not competing |
...
| Limitation | Papers Affected | How COLOSSUS v3 Addresses It |
|:-----------|:----------------|:-------------------------|
...
|:-------------|:-------|:-----------|
| "Speculative prefetching solves the problem" vs. "Speculation wastes bandwidth when wrong" | CommitMoE (#12) vs. MoE-Infinity (#10) | COLOSSUS v3 resolves via **confidence-gated speculation**: speculate when confident, demand-load when uncertain |
| "LRU is sufficient for expert caching" vs. "LRU fails under domain shifts" | Mixtral-Offloading (#9) vs. ADEPT (#13) | COLOSSUS v3: Least-Stale with **dynamic LRU-HP fallback** under high routing entropy |
| "Neuron-level sparsity only works with ReLU" vs. "SwiGLU has activation concentration" | TEAL (#35) vs. R-Sparse (#36) | COLOSSUS v3 empirically demonstrates 50% energy concentration in 115.5/768 columns with SwiGLU on Qwen3 |

### Open Research Questions (From "Future Work" Sections)

| Open Question | First Stated By | Status in COLOSSUS v3 |
|:-------------|:---------------|:-------------------|
...
## 11. Novelty Positioning: What COLOSSUS v3 Does That Nobody Else Does
...
### What Has NOT Been Done (COLOSSUS v3 Novelty)

| Novel Contribution | Why It's New | Closest Prior Work | Difference |
|:-------------------|:-------------|:-------------------|:-----------|
| **Column-level MoE serving** | All prior MoE serving operates at expert-level (9.44 MB). COLOSSUS v3 operates at column-level (30.72 KB) — the first sub-expert decomposition for MoE offloading | PowerInfer (dense models) | PowerInfer targets dense FFNs with ReLU; COLOSSUS v3 targets MoE experts with SwiGLU, using energy-ranked columns |
| **Intra-layer pre-attention prefetching** | All prior speculative prefetching predicts from the previous layer or token. COLOSSUS v3 predicts from the same layer's pre-attention state, hiding transfers behind the same layer's attention window | Pre-gated MoE (inter-layer) | Pre-gated MoE uses layer L-1 output; COLOSSUS v3 uses layer L input (pre-attention state $\mathbf{h}_{pre}$) |
| **Energy-based column priority scheduling** | No prior work ranks individual weight columns by activation energy to determine prefetch priority | Deja Vu (contextual sparsity) | Deja Vu predicts binary active/inactive neurons; COLOSSUS v3 ranks columns by continuous energy magnitude for priority queuing |
...
| **Lossless attention-window-hidden serving** | No prior system achieves 0% accuracy degradation while hiding 100% of weight transfers behind the attention compute window | CommitMoE (low stall but lossy) | CommitMoE drops 7/8 experts; COLOSSUS v3 fetches all via fast/slow paths |
| **Triton Gather-GEMM for indexed columns** | No existing kernel fuses indexed column gathering with GEMM and pointwise accumulation in a single pass | vLLM Fused MoE (full expert) | vLLM kernel processes full expert matrices; COLOSSUS v3 kernel gathers arbitrary column subsets from multiple sources |
...
## 12. Summary: Why COLOSSUS v3 Is New
...
**COLOSSUS v3 bridges this gap.** It is the first system to:
...
│                                                  │
│              ╔═══════════════╗                    │
│              ║   COLOSSUS v3     ║ ← Column-Level    │
│              ║  (This Work)  ║    MoE Serving     │
│              ╚═══════════════╝    (NEW)           │
│                                                  │
│   Lossy ─────────────── Lossless                 │
│   ● CommitMoE           ● COLOSSUS v3               │
