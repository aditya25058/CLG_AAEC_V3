...
# Rewritten: measure REAL latency breakdown via dual-stream timing and compare vs. full expert loading
import os
import sys
import json
import math
...
def run_latency_benchmark(model_name: str, spec: dict, device: str = "cuda:0"):
    ...
    # Full expert weights on CPU pinned memory (for baseline comparison)
    W_gate_f_cpu = torch.randn(I, H, dtype=torch.bfloat16).pin_memory()
    W_up_f_cpu   = torch.randn(I, H, dtype=torch.bfloat16).pin_memory()
    W_down_f_cpu = torch.randn(H, I, dtype=torch.bfloat16).pin_memory()
    ...
    W_gate_f_gpu = torch.empty(I, H, dtype=torch.bfloat16, device=device)
    W_up_f_gpu   = torch.empty(I, H, dtype=torch.bfloat16, device=device)
    W_down_f_gpu = torch.empty(H, I, dtype=torch.bfloat16, device=device)
    ...
    # --- Measurement 2: Full SA-FFN with real concurrent PCIe transfer ---
    def full_sa_ffn_with_pcie():
        # Start copying missed columns in parallel
        ...
        # Run Phase 1 compute on primary stream concurrently
    ...
    # --- Measurement 4: Raw PCIe transfer time only (Missed Columns) ---
    def pcie_only():
        with torch.cuda.stream(dma_stream):
            W_gate_m_gpu.copy_(W_gate_m_cpu, non_blocking=True)
            W_up_m_gpu.copy_(W_up_m_cpu, non_blocking=True)
    ...
    print(f"  Raw PCIe DMA transfer (Missed Columns): {t_pcie_raw:.4f} ms")

    # --- Measurement 5: Raw PCIe transfer time (Full Expert) ---
    def pcie_full_expert():
        with torch.cuda.stream(dma_stream):
            W_gate_f_gpu.copy_(W_gate_f_cpu, non_blocking=True)
            W_up_f_gpu.copy_(W_up_f_cpu, non_blocking=True)
            W_down_f_gpu.copy_(W_down_f_cpu, non_blocking=True)
        torch.cuda.current_stream().wait_stream(dma_stream)

    t_pcie_full_raw = timed(pcie_full_expert, device)
    print(f"  Raw PCIe DMA transfer (Full Expert): {t_pcie_full_raw:.4f} ms")

    # Compute real exposed stall and overlap fraction
    pcie_stall = max(0.0, t_full - t_no_pcie)
    
    # Overlap fraction: how much of the raw transfer was hidden behind compute
    # raw_transfer - exposed_stall = hidden_transfer
    hidden_transfer = max(0.0, t_pcie_raw - pcie_stall)
    overlap_fraction = hidden_transfer / max(t_pcie_raw, 1e-6)

    # Stall reduction vs loading a full expert
    # Demand-load full expert stall: transfer full expert, then run compute (no overlap baseline)
    full_expert_stall = t_pcie_full_raw
    stall_reduction = 1.0 - (pcie_stall / max(full_expert_stall, 1e-6))
    ...
    results = {
        ...
        "raw_pcie_full_expert_transfer_ms": round(t_pcie_full_raw, 4),
        "exposed_pcie_stall_ms": round(pcie_stall, 4),
        "pcie_overlap_ratio": round(overlap_fraction, 4),
        "stall_reduction_vs_full_expert": round(stall_reduction, 4),
        "miss_columns": M,
        "transfer_size_bytes": M * 3 * spec["hidden_size"] * 2
    }

    print(f"  Exposed PCIe Stall: {pcie_stall:.4f} ms")
    print(f"  Overlap Fraction: {overlap_fraction*100:.1f}%")
    print(f"  Stall Reduction vs Full Expert: {stall_reduction*100:.1f}%")
