# AAEC Evaluation Plan (OSDI/SOSP/MLSys Grade)
...
A top-tier systems paper must answer **six** distinct questions through its evaluation. Each question maps to a section below. Every experiment is designed to prove or disprove a specific hypothesis about AAEC's design.

---

## Section A: End-to-End Serving Benchmarks (The Bottom Line)

> **Question:** Does AAEC deliver faster, cheaper inference than existing systems on real workloads?
...
| **Mixtral-8x7B** | 46.7B | 12.9B | 8 per layer | 2 | Low-expert-count control (tests if AAEC helps when routing is already simple) |
...
| Layer-Heterogeneous LRU (AAEC) | Per-layer LRU with entropy-weighted capacity |
...
> **Question:** Does AAEC scale with model size, GPU count, batch size, and sequence length?
...
*   **Hypothesis:** AAEC's latency scales sub-linearly with model size because the active parameter footprint (and thus the transfer volume) grows much slower than total parameters in MoE architectures.
...
*   **Hypothesis:** AAEC scales near-linearly because weight-pulling traffic is independent of batch size, unlike EP's All-to-All which grows with GPU count.
...
*   **Hypothesis:** AAEC's PCIe traffic stays roughly constant (weight-pulling is per-layer, not per-token), while EP's All-to-All traffic scales linearly.
...
> **Question:** Does AAEC produce bit-identical outputs to the unmodified model?

### E.1 Exact Match Verification

| Test | Method | Target |
|---|---|---|
| **Logit Cosine Similarity** | Compare AAEC output logits vs. HuggingFace reference on 1000 ShareGPT prompts | **1.0000** (exact) |
| **Top-1 Token Agreement** | Fraction of tokens where AAEC and reference select the same next token | **100.00%** |
| **Perplexity Delta** | Compute perplexity on WikiText-2 for both AAEC and reference | **Δ = 0.0000** |
...
| Benchmark | Metric | AAEC Score | Reference Score | Delta |
|---|---|---|---|---|
...
> [!IMPORTANT]
> Because AAEC's SA-FFN is mathematically exact (all active neurons are computed, just in a different scheduling order), the accuracy delta **must be exactly zero** for all benchmarks. Any non-zero delta indicates a bug, not an inherent trade-off.

---

## Section F: Sensitivity Analysis (Robustness)

> **Question:** How sensitive is AAEC to its configuration parameters?
...
*   **Plot:** Decode latency vs. link bandwidth for AAEC and all baselines.
*   **Find:** The crossover point where AAEC's prefetching advantage disappears (i.e., the link is fast enough that demand-driven fetching has no stalls).
...
|---|---|
| **A (End-to-End)** | AAEC delivers lower latency and higher throughput than all baselines on real workloads |
...
| **D (Scalability)** | AAEC scales with model size, GPU count, batch size, and sequence length |
| **E (Accuracy)** | AAEC is provably, bit-for-bit lossless |
| **F (Sensitivity)** | AAEC is robust across a wide range of configuration parameters and hardware conditions |
