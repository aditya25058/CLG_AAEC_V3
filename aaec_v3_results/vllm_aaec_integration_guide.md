# Systems Integration Guide: Implementing AAEC in vLLM

To validate AAEC in a production inference stack and publish at a top systems venue (OSDI, SOSP, NSDI, EuroSys), we must move from an offline PyTorch benchmark to an online integration. This guide provides a concrete, step-by-step engineering plan to integrate AAEC into the **vLLM** serving framework.

---

## Architectural Overview: AAEC in vLLM

To implement AAEC, we must intercept the data flow at three layers of the vLLM engine:
...
nsys profile --trace=cuda,nvtx,pcie --output=aaec_telemetry \
    python3 -m vllm.entrypoints.openai.api_server \
    --model deepseek-ai/DeepSeek-V2-Lite \
    --device-mapping custom_aaec_map
```
...
*   **Perplexity (PPL):** Evaluated on WikiText-2 to confirm that offloaded AAEC serving matches GPU-native perplexity.
