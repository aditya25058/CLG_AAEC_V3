...
def main():
    ...
    for model_name, spec in MODELS.items():
        print(f"Running Scalability Sweep for {model_name}...")
        calib_db, eval_db = load_db_traces(spec["db_path"])
        trans_matrix, top_cols, l0_freq = train_predictors(calib_db, spec)
        
        results = {}
        for conn_name, bw in interconnects.items():
            results[conn_name] = {}
            print(f"  Interconnect: {conn_name} ({bw} GB/s)...")
            for size in cache_sizes:
                hr, stall = run_scalability_simulation(eval_db, trans_matrix, top_cols, l0_freq, cache_size=size, spec=spec, link_bw_gb_s=bw)
                results[conn_name][str(size)] = {
                    "hit_rate": hr,
                    "avg_stall_ms": stall
                }
                print(f"    Cache Size: {size:<3} | Hit Rate: {hr*100:.2f}% | Stall: {stall:.4f} ms")
                
        out_dir = f"/home/palakm/MoEServingSim/evaluation/results/e12_scalability/{model_name}"
