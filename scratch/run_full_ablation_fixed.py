        ...
        for t in calibration_db[p_id]:
            for l in range(NL):
                if l in calibration_db[p_id][t]:
                    active_experts = calibration_db[p_id][t][l]
                    for exp_id, active_set in active_experts:
                        if exp_id < NE:
        ...
        for idx, t in enumerate(t_positions):
            ...
            for l in range(NL):
                if l not in evaluation_db[p_id][t]:
                    continue
                active_experts = evaluation_db[p_id][t][l]
                
                cache = gpu_caches[l]
                
                # Retrieve the set of keys we want to access at this step
                if config == "no_aaec":
                    active_keys = {exp_id for exp_id, _ in active_experts}
                else:
                    active_keys = {(exp_id, col) for exp_id, cols in active_experts for col in cols}
            ...
            if idx < len(t_positions) - 1:
                ...
                for l in range(NL):
                    # For Qwen3/DeepSeek, we prefetch top columns of the predicted experts
                    # Predict active experts using transition matrix
                    pred_exps = []
                    if l == 0:
                        pred_exps = [layer_0_most_frequent]
                    else:
                        if (l-1) in evaluation_db[p_id][t]:
                            # Union over predicted next experts for all active experts of previous layer
                            prev_exps = [exp_id for exp_id, _ in evaluation_db[p_id][t][l-1] if exp_id < NE]
                            if prev_exps:
                                # Take top predicted next expert for each active previous expert
                                for pe in prev_exps:
                                    pred_exps.append(int(np.argmax(transition_matrix[l, pe])))
                                pred_exps = list(set(pred_exps))
                            else:
                                pred_exps = [0]
                        else:
                            pred_exps = [0]
                    ...
                    for pe in pred_exps:
                        temp_cols = prev_token_active_cols.get((l, pe), set())
                        pred_cols_set = {(pe, col) for col in temp_cols}
                        static_cols = set([(pe, col) for col in top_cols_per_expert[(l, pe)][:cache_size]])
                        predicted_keys = predicted_keys.union(pred_cols_set).union(static_cols)
...
def main():
    ...
    for model_name, spec in MODELS.items():
        print(f"Running Fixed Ablation Sweep for {model_name}...")
        ...
        with open(os.path.join(out_dir, "full_ablation_results_fixed.json"), "w") as f:
