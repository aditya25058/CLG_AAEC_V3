...
def main():
    ...
    for bw in bandwidths:
        ...
        temp_config_path = f"configs/cluster/temp_epeg_bw_{str(bw).replace('.', '_')}.json"
        with open(temp_config_path, "w") as f:
            json.dump(temp_config, f, indent=4)
            
        for enabled in epeg_options:
            enabled_str = "enabled" if enabled else "disabled"
            csv_out = f"outputs/phase3/epeg_{enabled_str}_bw_{str(bw).replace('.', '_')}.csv"
            ...
            print(f"EPEG Sweep: BW={bw} GB/s | EPEG={enabled_str.upper()} -> Latency: {metrics['total_latency_s']}s, TTFT: {metrics['avg_ttft_ms']}ms, TPOT: {metrics['avg_tpot_ms']}ms")
            
        if os.path.exists(temp_config_path):
            os.remove(temp_config_path)

    # Save results to JSON
    with open("outputs/phase3/epeg_results.json", "w") as f:
        json.dump(results, f, indent=4)
        
    print("Successfully completed EPEG sweeps and wrote outputs/phase3/epeg_results.json")
