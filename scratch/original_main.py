...
def main():
    ...
    parser.add_argument('--expert-routing-policy', type=str,
                        choices=['BALANCED', 'RR', 'RAND', 'CUSTOM', 'DATASET'],
                        default='BALANCED',
    ...
    if enable_prefix_caching and enable_prefix_sharing and prefix_storage != 'None':
        ...
        def _pool_kv_bytes_per_token(inst_ids):
            ...
            inst = instances[inst_ids[0]]
            model = inst['model_name']
            tp_size = inst.get('tp_size', 1)
            return full_cluster_kv_bytes_per_token(model, fp, kv_cache_dtype, tp_size=tp_size)
