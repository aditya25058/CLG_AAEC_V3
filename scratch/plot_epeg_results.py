...
# Load data dynamically from epeg_results.json
results_json_path = "outputs/phase3/epeg_results.json"
if not os.path.exists(results_json_path):
    print(f"Error: {results_json_path} not found. Please run scratch/run_epeg_sweeps.py first.")
    exit(1)
...
plt.suptitle("Elastic-Precision Expert Gating (EPEG) Performance Gains (Sweep 4)", fontsize=16, fontweight='bold', y=0.98)
plt.tight_layout()

# Save plot
plot_path = "outputs/phase3/epeg_comparison_plot.png"
plt.savefig(plot_path, dpi=300, bbox_inches='tight')
plt.close()
print(f"Successfully saved EPEG curves plot to {plot_path}")
...
ax_tbl.set_title("EPEG Performance Gains and Speedup Scorecard", fontsize=14, fontweight='bold', pad=20)
plt.tight_layout()

# Save table
table_path = "outputs/phase3/epeg_results_table.png"
plt.savefig(table_path, dpi=300, bbox_inches='tight')
plt.close()
print(f"Successfully saved EPEG table to {table_path}")
