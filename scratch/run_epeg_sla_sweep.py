...
def calculate_stats(records, variant):
    ...
    # Static EPEG: (0.40, 0.05) -> Loss: 0.000540 -> GSM8K: 0.0810%, MMLU: 0.0540%, LCB: 0.1081%
    # EPEG-SLA: (0.50, 0.10) for congested steps, (0.0, 0.0) for uncongested steps.
    # We project the overall run loss as:
    # Baseline: 0.0% loss
    # Static EPEG: 100% of requests have static EPEG loss
    # EPEG-SLA: congested requests have SLA (0.50, 0.10) loss, others have 0%
    if variant == "baseline":
        gsm = 0.0
        mmlu = 0.0
        lcb = 0.0
...
def main():
    ...
    sweeps = [
        {
            "name": "low_load",
            "dataset": "datasets/qwen3_remote_10req.jsonl",
            "num_reqs": 10
        },
        {
            "name": "high_load",
    ...
    for sweep in sweeps:
        ...
        for variant in variants:
            var_id = variant["id"]
            csv_out = f"outputs/phase3/epeg_sla_{sweep_name}_{var_id}.csv"
            
            cmd = [
                venv_python, "-m", "serving",
                "--cluster-config", cluster_config,
                "--dataset", sweep["dataset"],
                "--num-reqs", str(sweep["num_reqs"]),
                "--expert-routing-policy", "DATASET",
                "--gpus-per-node", "8",
                "--output", csv_out
            ] + variant["flags"]
            
            # Avoid terminal deadlock: do NOT add --end-npu-ids as requested by guidelines
            
            stdout = run_cmd(cmd)
            metrics = parse_metrics(stdout)
            
            if metrics["total_latency_s"] == 0.0:
                print(f"ERROR: Simulation failed for {sweep_name} / {var_id}")
                continue
            ...
            results[sweep_name][var_id] = combined
            ...
            print(f"   Congested Reqs: {combined['congested_reqs_count']}/{sweep['num_reqs']} ({combined['congested_reqs_pct']:.1f}%)")
    ...
    print(f"\n======================================================================")
    print(f"Successfully completed EPEG-SLA sweeps! Wrote results to {results_path}")
    print(f"======================================================================")
