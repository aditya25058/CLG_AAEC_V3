...
def main():
    ...
    for row in eval_rows:
        p_id, t_pos, layer, exp_id, _, _ = row
        ...
        if exp_id in cache:
            ...
            mono_misses += 1
            mono_miss_sizes.append(768)  # Full expert is 768 columns
    ...
    total_steps = len(eval_rows)
    mono_miss_rate = mono_misses / total_steps * 100
    col_miss_rate = len(col_miss_sizes) / total_steps * 100
    
    print("=" * 80)
    print(" EMPIRICAL ABLATION DIAGNOSTICS FOR QWEN3-30B")
    print("=" * 80)
    print(f"Total layer execution steps evaluated: {total_steps}")
    print("\n--- CONFIGURATION 1: MONOLITHIC CACHING (No AAEC) ---")
    print(f"Total layer steps with cache misses: {mono_misses} ({mono_miss_rate:.2f}%)")
